# Hi, I'm Mrun 👩‍💻 :)

I'm a computational biology nerd who likes asking cells weird questions.

### Currently
- Computational biology intern @ **OPTin** (Organ Pathobiology and Therapeutics Institute, University of Pittsburgh) 
- Master's student in **Quantitative Biology & Bioinformatics (MSQBB)** @ Carnegie Mellon

### Tools
- Programming: **Python**, **R**, **MATLAB** and **Bash**
- Machine Learning: XGBoost, Gradient Boosting, kNN, PCA, scikit-learn
- Bioinformatics: BEDTools, BioPython, BLAST, g:Profiler, Enrichr, IPA

### Things I nerd out about
- **Graphs everywhere** - GNNs, graph theory, graph algorithms. If it can be drawn as nodes and edges, I probably want to talk about it.
- Making little **Arduino projects** that try to embody bioinformatics models in hardware (turns out neurons are easier to simulate when you can poke them with a wire)
- Generally: anything where biology gets to wear a math hat 


Check out more at mrunmayeewankhede.com 😁!


